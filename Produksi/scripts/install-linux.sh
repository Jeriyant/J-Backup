...
for runtime_path in \
  "${install_dir}" "${data_dir}" "${rsync_dir}" "${backup_dir}" "${log_dir}"; do
  if [[ "${runtime_path}" == *$'\n'* || "${runtime_path}" == *'"'* ]]; then
...
if command -v apt-get >/dev/null 2>&1; then
  ...
  apt-get install -y \
    ...
    rsync p7zip-full openssh-client sshpass acl ca-certificates curl
  web_user="www-data"
  ...
  dnf install -y \
    ...
    rsync p7zip p7zip-plugins openssh-clients sshpass acl ca-certificates curl
  web_user="apache"
...
install -d -m 0775 -o root -g "${web_user}" "${install_dir}"
...
chown root:"${web_user}" "${install_dir}"
for code_path in src assets bin deploy scripts index.php api.php version.json og.png .htaccess "Cara Install.md"; do
  if [[ -e "${install_dir}/${code_path}" ]]; then
    chown -R root:"${web_user}" "${install_dir}/${code_path}"
    chmod -R g+rwX "${install_dir}/${code_path}"
  fi
done
install -d -m 0770 -o root -g "${web_user}" \
  "${data_dir}" "${data_dir}/staging" "${log_dir}"
...
chmod 0775 "${install_dir}/scripts/update-linux.sh"
...
render_template() {
  ...
  JBACKUP_TEMPLATE_RSYNC_DIR="${rsync_dir}" \
  ...
  php -r '
    ...
    $replacements = [
        ...
        "@@RSYNC_DIR@@" => (string) getenv("JBACKUP_TEMPLATE_RSYNC_DIR"),
